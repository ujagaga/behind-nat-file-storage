...
#include "mongoose.h"
#include "dir_list_start.h"
#include "dir_list_end.h"
...
int mg_http_get_var(const struct mg_str *buf, const char *name, char *dst,
  ...
  if (dst == NULL || dst_len == 0) {
    ...
    for (p = buf->ptr; p + name_len < e; p++) {
      if ((p == buf->ptr || p[-1] == '&' || p[-1] == '?') && p[name_len] == '=' &&
...
int mg_quick_http_upload(struct mg_connection *c, struct mg_http_message *hm, const char *dir, char* success_msg) {
  char offset[40] = "", name[200] = "", path[256];
  mg_http_get_var(&hm->query, "offset", offset, sizeof(offset));
  mg_http_get_var(&hm->query, "name", name, sizeof(name));
  if (name[0] == '\0') {
    ...
    if ((fp = mg_fopen(path, oft == 0 ? "wb" : "ab")) == NULL) {
      ...
      if(success_msg == NULL){
        ...
        mg_http_reply(c, 200, "", success_msg);
      }
...
static void listdir(struct mg_connection *c, struct mg_http_message *hm,
  ...
  if ((dirp = (opendir(dir))) != NULL) {
    ...
    mg_printf(c, dir_list_html1, (int) hm->uri.len, hm->uri.ptr, c->label);
